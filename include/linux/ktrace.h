...
/*
 *  * sed -i 's/^\([^\[]*\[[0-9]\{3\}\] [^\s]\{4\} \s*[0-9]\+\.[0-9]\+:\)
 *   *\([^:]*:\) \(tracing_mark_write: .*\)/\1 \3/g' <trace>.html
 *    */
...
#else
static inline void ktrace_add_mm_event(unsigned char type, u64 time_stamp,
						       u64 delta) {}
static inline void ktrace_print(const char *fmt, ...) {}
static inline void ktrace_add_sched_event(unsigned char type, pid_t pid,
							  u64 time_stamp, u64 delta,
							  					  void *pc) {}
static inline void ktrace_add_binder_event(unsigned char type, u64 time_stamp,
							   u64 delta) {}
...
static inline void ktrace_add_cpufreq_event(unsigned char type,
							    pid_t pid, u64 time_stamp,
							    					    unsigned int cpu,
...
static inline void ktrace_cpufreq_set_mitigated(char *comm, unsigned int cpu,
								const struct cpumask *related_cpus,
														unsigned int max){}
static inline void ktrace_cpufreq_update_history(struct task_struct *p,
